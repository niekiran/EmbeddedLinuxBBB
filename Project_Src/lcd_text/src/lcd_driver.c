...
#if 0
void lcd_process_command(uint8_t command)
{

	switch(command)
	{
	case LCD_CLEAR_DISPALY:
		lcd_send_command(LCD_CLEAR_DISPALY);
		break;
	case LCD_MOV_2ND_ROW:
		lcd_send_command(LCD_MOV_2ND_ROW);
		break;
	case LCD_MOV_HOME:
		lcd_send_command(LCD_MOV_HOME);
		break;
	case LCD_LEFT_SHIFT:
		lcd_send_command(LCD_LEFT_SHIFT);
		break;
	case LCD_RIGHT_SHIFT:
		lcd_send_command(LCD_RIGHT_SHIFT);
		break;
	case LCD_DISP_ON:
